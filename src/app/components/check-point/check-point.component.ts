...
export class CheckPointComponent implements OnInit {
  ...
  // meows = [
  //   new Audio("../../../assets/meows/meow.ogg"),
  ...
  addPointsFromForm() {
    ...
    for (let i = 0; i < this.selectedR.length; i++) {
      for (let j = 0; j < this.selectedX.length; j++) {
        ...
        try {
          this.point.y = calculate(this.selectedY, this.selectedY);
        } catch (e) {
    ...
    if (!isNaN(intR)) {
      if (this.AVAILABLE_R.indexOf(intR) !== -1) {
        ...
        if (this.selectedR.length < 1) {
          ...
          this.clearCtx();
          for (let i = 0; i < this.selectedR.length; i++) {
            console.log('draw with', this.selectedR[i]);
            this.draw(this.selectedR[i], (i + 1) / this.selectedR.length);
            this.getPointsRecalculated(this.selectedR[i]);
